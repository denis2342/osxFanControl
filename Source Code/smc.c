...
int main(int argc, char *argv[])
{
    ...
    temps[0] = temps[1] = temps[2] = temps[3] = temps[4] = temps[5] = 50;
    ...
    while (1) {
        ...
        temp = SMCReadTemperature(SMC_KEY_CPU_A_TEMP);
        ...
        if (temp != 0.0) {
            ...
            temp = (temps[0] + temps[1] + temps[2] + temps[3] + temps[4] + temps[5]) / 6.0;

           int rpm = 600 + (temp - 50.0) * (2800 - 600) / 30;
            ...
            if (SMCSetFanRpm(SMC_KEY_INTAKE_RPM_MIN, rpm) != kIOReturnSuccess) {
                printf("Error: SMCWriteKey() = %08x\n", result);
            }

            if (SMCSetFanRpm(SMC_KEY_CPU_A_RPM_MIN, rpm) != kIOReturnSuccess) {
                printf("Error: SMCWriteKey() = %08x\n", result);
            }
            
            if (SMCSetFanRpm(SMC_KEY_EXHAUST_RPM_MIN, rpm) != kIOReturnSuccess) {
                printf("Error: SMCWriteKey() = %08x\n", result);
            }
        }
        SMCClose();
